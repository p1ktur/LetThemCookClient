...
import com.letthemcook.feed.data.LocalFavoredRecipeDataManager
import com.letthemcook.feed.data.database.FavoredRecipeDatabase
...
val koinFeedModule = module {
    single { FavoredRecipeDatabase.getInstance(androidContext()) }
    single { get<FavoredRecipeDatabase>().getDao() }
    single { LocalFavoredRecipeDataManager(get()) }
